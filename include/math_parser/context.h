...
struct Context {
    public:
        ...
        virtual double call_function(std::string name, double arguments[], uint argumentCount) = 0;
};

struct DefaultContext : public Context {
    public:
        double resolve_constant(std::string name);
        double call_function(std::string name, double arguments[], uint argumentCount);
};
